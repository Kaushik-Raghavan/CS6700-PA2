...
COLORS = {0:[1.0,1.0,1.0], 1:[0.6,0.6,0.6], \
		...
		4:[0.0,0.0,1.0], 5:[1.0,0.0,1.0], \
		6:[0.1,0.1,0.1], 7:[0.0,0.0,1.0]}
# Colors 1, 2, 3 corresponds to puddle
# Color 4 corresponds to terminal state
# Color 5 corresponds to the state of the agent
# All other colors are irrelevant here
...
class PuddleWorldEnv(gym.Env):
	...
	def __init__(self):
		...
		self.reward_range = (-3, 10)

		self.obs_shape = [121, 121, 3]
		self.state_reward = [-0.5, -1.0, -2.0, -3.0, +10.0, 0, 0, 0]
	...
	def step(self, action):
		...
		if (np.random.binomial(1, 0.9 - (0.1 / 3.0)) == 0):  
		...
		if (np.random.rand() < 0.5):
			# With probability 0.5, wind moves the agent
			nxt_state = nxt_state[0], max(nxt_state[1] - 1, 0)
		...
		if (self.verbose and nxt_state != self.state):  # Update image only if the state has changed
			...
			self.current_grid_map[nxt_state[0], nxt_state[1]] = 5
	...
	def reset(self):
		...
		self.current_grid_map[self.state[0], self.state[1]] = 5
		self.observation = self._gridmap_to_observation(self.start_grid_map)
		self.done = False
		self.render()
		return self.state
