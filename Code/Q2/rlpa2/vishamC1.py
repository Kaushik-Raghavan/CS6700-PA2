...
class vishamC1(Env):
	...
	def _step(self, action):
		...
		coeff = 10
		rwd = -0.5 * (new_state[0]**2) - 0.5 * coeff * (new_state[1]**2)
		if (abs(new_state[0]) > 1 or abs(new_state[1]) > 1):
			# If it goes out of the environment, the corresponding negative reward is given, and the episode ends
			state = self._reset()
			return state, rwd, False, {}

		self.state = new_state
		self.done = (np.linalg.norm(new_state) <= 5e-2)
		if self.done:
			rwd = +2.0
		return self.state, rwd, self.done, {} 
...
register(
		id='vishamC1-v0',
		entry_point='rlpa2.vishamC1:vishamC1',
		#timestep_limit=40,
		)
